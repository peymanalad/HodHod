(function () {
  app.modals.InactivityControllerNotifyModal = function () {
      ...
      abp.services.app.session
        ...
        .done(function (info) {
          if (info && info.user && info.user.id) {
            getExternalLoginProviderTypeName(info.user.id, function (externalLoginProviderName) {
    ...
    function getExternalLoginProviderTypeName(userId, callback) {
      abp.services.app.userLogin.getExternalLoginProviderNameByUser(userId).done(function (providerName) {
        if (typeof callback === 'function') {
