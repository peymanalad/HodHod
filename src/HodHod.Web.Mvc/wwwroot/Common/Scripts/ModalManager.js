...
(function ($) {
  ...
  app.ModalManager = (function () {
    ...
    return function (options) {
      ...
      var _getMinifiedUrl = function (url) {
        return url.endsWith('.min.js') ? url : url.replace(/\.js$/, '.min.js');
      };
      ...
      var _open = function (args, getResultCallback, onShownCallback) {
        ...
        _createContainer(_modalId, _modalSize)
          .find('.modal-content')
          .load(_options.viewUrl, _args, function (response, status, xhr) {
            if (status == 'error') {
            ...
            if (_options.scriptUrl) {
              _options.scriptUrl = _getMinifiedUrl(_options.scriptUrl);

              app.ResourceLoader.loadScript(_options.scriptUrl, function () {
                _initAndShowModal();
              });
            } else {
              _initAndShowModal();
