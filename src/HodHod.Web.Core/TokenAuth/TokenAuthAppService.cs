...
public class TokenAuthAppService : HodHodAppServiceBase, ITokenAuthAppService
{
    ...
    private async Task<string> EncryptQueryParameters(long userId, Tenant tenant, string passwordResetCode)
    {
        ...
        var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
            .ToString(HodHodConsts.DateTimeOffsetFormat));
    ...
    public async Task<TokenAuth.RefreshTokenResult> RefreshToken(string refreshToken)
    {
        ...
        try
        {
            ...
            return await Task.FromResult(new TokenAuth.RefreshTokenResult(
                accessToken,
    ...
    public async Task LogOut()
    {
        if (AbpSession.UserId != null)
        {
            ...
            {
                var tokenValidityKeyInClaims = userPrincipal.Claims.First(c => c.Type == AppConsts.TokenValidityKey);
                await RemoveTokenAsync(tokenValidityKeyInClaims.Value);

                var refreshTokenValidityKeyInClaims =
                    userPrincipal.Claims.FirstOrDefault(c => c.Type == AppConsts.RefreshTokenValidityKey);
    ...
    [AbpAuthorize]
    public async Task<string> TestNotification(string message = "", string severity = "info")
    {
